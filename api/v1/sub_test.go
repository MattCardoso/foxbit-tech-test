...
func TestSubHandleFuncOkPath(t *testing.T) {
	tt := []struct {
		t1       string
		t2       string
		expected map[string]int
	}{
		{t1: "5", t2: "5", expected: map[string]int{"result": 0}},
		{t1: "5", t2: "0", expected: map[string]int{"result": 5}},
	}
	for _, tc := range tt {
		path := fmt.Sprintf("/api/sub?term_one=%s&term_two=%s", tc.t1, tc.t2)
		...
		mux.HandleFunc("GET /api/sub", Sub)
...
func TestSubHandleFuncWrongPathEmptyValues(t *testing.T) {
	tt := []struct {
		t1            string
		t2            string
		expectedCode  int
...
func TestSubHandleFuncWrongPathNonNumberValues(t *testing.T) {
	tt := []struct {
		t1            string
		t2            string
		expectedCode  int
