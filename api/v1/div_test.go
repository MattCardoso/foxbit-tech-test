...
func TestDivHandleFuncOkPath(t *testing.T) {
	tt := []struct {
		t1       string
		t2       string
		expected map[string]int
	}{
		{t1: "5", t2: "5", expected: map[string]int{"result": 1}},
		{t1: "0", t2: "5", expected: map[string]int{"result": 0}},
	}
	for _, tc := range tt {
		path := fmt.Sprintf("/api/div?term_one=%s&term_two=%s", tc.t1, tc.t2)
		...
		mux.HandleFunc("GET /api/div", Div)
...
func TestDivHandleFuncWrongPathEmptyValues(t *testing.T) {
	tt := []struct {
		t1            string
		t2            string
		expectedCode  int
...
func TestDivHandleFuncWrongPathNonNumberValues(t *testing.T) {
	tt := []struct {
		t1            string
		t2            string
		expectedCode  int
...
func TestDivHandleFuncWrongPathZeroDivider(t *testing.T) {
	tt := []struct {
		t1            string
		t2            string
		expectedCode  int
		...
		{t1: "12", t2: "0", expectedCode: http.StatusBadRequest, expectedError: pkg.ErrDivideByZero.Error() + "\n"},
	}
	for _, tc := range tt {
		path := fmt.Sprintf("/api/div?term_one=%s&term_two=%s", tc.t1, tc.t2)
		req, err := http.NewRequest("GET", path, nil)
